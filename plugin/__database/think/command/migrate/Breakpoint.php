...
class Breakpoint extends Migrate
{
    ...
    {
        $this->setName('panda:migrate-breakpoint')
             ...
             ->setHelp(<<<EOT
                 The <info>breakpoint</info> 
                 command allows you to set or clear a breakpoint against a specific target to inhibit rollbacks beyond a certain target.
...
EOT
    ...
    protected function toggleBreakpoint($version)
    {
        ...
        $this->output->writeln(
            ...
            ' for <info>' . $version . '</info>' . ' <comment>' . $migrations[$version]->getName() . '</comment>'
        );
    }
